...
> **Stop guessing, start cutting.** A professional, no-sign-up 2D bin packing calculator that generates precise cutting diagrams for plywood, lumber, and sheet goods — 100% in your browser.
...
**FreeCutList.app** is a high-performance **cut list calculator** and **panel cutting layout optimizer** built specifically for woodworkers, cabinet makers, and DIY enthusiasts. 

It bridges the gap between expensive legacy desktop software (like CutList Plus) and the need for a modern, private, and zero-registration web-based solution. Feed it your part dimensions, pick your stock sheets, and it generates an optimized **cutting diagram** that minimizes waste while respecting physical constraints like grain direction and saw blade kerf.

No account. No sign-up. No cloud. Every calculation runs **100% locally** in your browser via a dedicated Web Worker thread. 
...
Unlike generic rectangle-packing tools, every feature is calibrated for real-world woodworking projects:
...
| **15 Packing Heuristics** | The engine cross-iterates 15 guillotine-cut strategies with Lazy Merge optimization, packing hundreds of parts in milliseconds. |
...
| **Math Core** | Proprietary fraction-snapping engine — eliminates IEEE-754 float drift on imperial dimensions |
...
| **Styling** | Tailwind CSS + hand-tuned native CSS for commercial-grade PDF exports |

---

## 📦 Project Structure (Open-Core)

This repository publishes the core template data and utility modules that power the application. The production packing engine and application shell remain proprietary.
...
├── public/
│   └── templates/            # Static SEO landing pages for specialized projects
│       ├── index.html        # Template gallery hub
│       ├── 36-inch-base-cabinet.html
│       ├── garage-storage-shelves.html
│       └── … (38 more)
└── docs/
    └── screenshot-hero.png   # High-resolution app preview
```
...
Browse our library of pre-loaded project templates. Each includes a verified cut list, 3D-exploded view diagram, and optimized layout.
...
### 📋 Full Template Index (SEO Hub)

<details>
<summary><b>Expand all 40+ project templates</b></summary>
...
The **woodworking template data** (`/data/`) and **fractional utility modules** (`/src/utils/`) are released under the [MIT License](https://opensource.org/licenses/MIT). 

The core packing engine and application shell are proprietary. You are free to use the project dimensions and utilities in your own projects, including commercial use.
